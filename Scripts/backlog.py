...
class Task:
    ...
    def total(self, unblocks: int) -> float:
        ...
        # `perf` is how much this moves frame time, added 2026-07-31 when the owner
        # made performance priority 1 of 1. Deliberately NOT in SCORE_KEYS: it is
        # optional and defaults to 1, so every task filed before it keeps its exact
        # score and nothing re-ranks. Declare 2 or 3 only when the task actually
        # buys frame time — a perf multiplier on everything is the same as none.
        perf = self.score_input("perf", 1)
        return round((feel * risk * perf * unblocks) / cost, 2)
...
def cmd_reindex(args) -> int:
    ...
    out.append("")
    out.append("Score is `(feel × risk × perf × unblocks) ÷ cost`, where **`feel` is how much")
    out.append("this changes the moment-to-moment feel or gameplay** (owner, 2026-07-28) and")
    out.append("**`perf` is how much it buys frame time** (owner, 2026-07-31 — performance is")
    out.append("priority 1 of 1). `perf` is optional and defaults to 1, so tasks filed before")
    out.append("it keep their exact score; declare 2 or 3 only when the task really moves")
    out.append("frame time. The inputs are printed beside the total so a disagreement costs")
    out.append("one sentence, not a re-read.")
    out.append("")
    ...
    if not queue:
        ...
        out.append("| # | score | f×r×p×u÷c | task | agent | cost | evidence on done |")
        out.append("|---|---|---|---|---|---|---|")
        for i, (total, unb, t) in enumerate(queue, 1):
            out.append(
                f"| {i} | **{total}** | "
                f"{t.score_input('feel')}×{t.score_input('risk')}×{t.score_input('perf')}"
                f"×{unb}÷{t.score_input('cost')} | "
