...
## Fill `rows` ranks, `half` world units either side of centre, first rank at depth `d0` (rows step toward the camera).
static func block(host: Node2D, parent: Node2D, mix: Dictionary, half: float, rows: int, d0: float, rng: RandomNumberGenerator, d_offset := 0.0) -> Array[Unit]:
	...
	for k in range(per_row * rows):
		...
		u.wx = -half + col * RANK_X + (row % 2) * RANK_X * 0.5 + rng.randf_range(-3, 3)
		u.wd = d_offset + d0 - row * RANK_STEP + rng.randf_range(-2, 2)
		u.hold_d = u.wd
