...
| 29 | Command granularity (how many handles, and what they address) | **Command by unit TYPE, not by group** — "all archers" / "all spearmen" / "all healers" (§4). Handle count scales with the number of unit types, so it never folds. **Supersedes the fixed `MaxSquads = 8` group model** (`docs/design/squad-group-system.md` §4.2-§4.3, `SwarmSubsystem.h:46`), which folds recruits into unit 0 at roughly 730 retinue — a projection, UNVERIFIED, and disqualifying anyway under a persistent army that only grows (Q22). | ✅ Decided 2026-07-31 |
