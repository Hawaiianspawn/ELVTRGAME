...
| C10 | `wave-scaling.json`:23 argues from "`economy.json` supply.start_capacity (60)"; task-101 moved it to **120** the same day (`economy.json`:9). The no-degrade-at-wave-1 argument no longer holds as written. **STATUS: ✅ APPLIED 2026-07-31.** The wave-1 note now carries a `STALE, CORRECTED 2026-07-31` marker quoting the retired sentence and stating that the *conclusion* survives (60 demand under 120 capacity still does not degrade) but the *reasoning* does not. The 34,000 divisor in `design_constants` is marked UNVERIFIED in the same pass, and `wave-scaling.schema.md`:32 now defines `HeadroomPctVs34kCeiling` as arithmetic rather than measurement (C3). |
| C11 | `INDEX.md`:6 ranks by `(feel × risk × unblocks) ÷ cost` with **no performance term**. "Priority 1 of 1" that is not in the ranking rule gets silently re-derived away. Add a performance term. **STATUS: ✅ APPLIED 2026-07-31** (owner said dispatch). Score is now `(feel × risk × perf × unblocks) ÷ cost`. `perf` is deliberately **not** in `SCORE_KEYS` — it is optional and defaults to 1, so all 133 existing tasks keep their exact score and nothing re-ranks except where `perf` is declared. Validate passes 0 errors. task-108 declared `perf: 3` and moved 3.0 → 9.0, **7th → 2nd**. |
| C12 | **Un-park task-105.** Under this direction it is the single highest-value backlog item — the only task that puts a small starting army and a massive ending one on screen. **STATUS: ⛔ DISPATCHED 2026-07-31, BLOCKED BY LOCK — not an omission.** Owner said dispatch; `backlog.py approve` refused **both** 105 and 108: task-133 (*Make the camera contain every live unit*) is `in-progress` with an in-process teammate and holds the `unreal-editor` lock plus `ELVTR/Source/ELVTR/Spike/**`, which overlaps 105's `owns`. Parking a peer session's live work to clear the lock was declined ([[concurrent-sessions-share-the-tree]]). **Both re-approve the moment task-133 closes** — no other blocker. task-108's `perf: 3` was applied anyway, so it re-enters the queue at #2 rather than #7. |
...
| file:line | What breaks | Required change |
|---|---|---|
| `INDEX.md`:6-9 | "Score is `(feel × risk × unblocks) ÷ cost`, where **`feel` is how much this changes the moment-to-moment feel or gameplay** (owner, 2026-07-28)." **No performance term.** | **C11. ✅ APPLIED 2026-07-31.** `Scripts/backlog.py` `Task.total()` now multiplies by `perf`, an **optional** input defaulting to 1 — chosen over adding it to `SCORE_KEYS` precisely so 133 existing task files neither fail validation nor silently re-rank. Formula, display (`show`, `list`, INDEX table header `f×r×p×u÷c`) and the generated INDEX preamble all updated together, so the printed rule matches the computed one. Declare `perf: 2`/`3` only when a task really buys frame time — a multiplier on everything is the same as none. |
